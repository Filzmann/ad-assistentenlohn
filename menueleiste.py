...
class Menuleiste(tk.Menu):
    def __init__(self, parent):
        ...
        eintragen_menu.add_command(label="Urlaub eintragen", command=lambda: NeuerUrlaub(root, assistent))
        eintragen_menu.add_command(label="AU/krank eintragen", command=lambda: NeueAU(root, assistent))

        bearbeiten_menu = tk.Menu(self, tearoff=0)
        bearbeiten_menu.add_command(label="ASN bearbeiten", command=lambda: FensterEditAsn(root, assistent))
        bearbeiten_menu.add_command(label="Assistent bearbeiten", command=lambda: FensterNeuerAS(root,
                                                                                                 assistent,
                                                                                                 edit=1))
